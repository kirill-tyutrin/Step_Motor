...
#define LL_GPIO_ReadReg(__INSTANCE__, __REG__) READ_REG(__INSTANCE__->__REG__)
	...
	__STATIC_INLINE void LL_GPIO_SetPinOutputType(GPIO_TypeDef *GPIOx,
			uint32_t Pin, uint32_t OutputType) {
		register uint32_t *pReg =
				(uint32_t*) ((uint32_t) ((uint32_t) (&GPIOx->CRL) + (Pin >> 24)));
	...
	__STATIC_INLINE uint32_t LL_GPIO_GetPinPull(GPIO_TypeDef *GPIOx,
			uint32_t Pin) {
		return (READ_BIT(GPIOx->ODR,
				(GPIO_ODR_ODR0 << (POSITION_VAL(Pin >> GPIO_PIN_MASK_POS))))
	...
	__STATIC_INLINE uint32_t LL_GPIO_AF_IsEnabledRemap_SPI1(void) {
		return (READ_BIT(AFIO->MAPR, AFIO_MAPR_SPI1_REMAP)
	...
	__STATIC_INLINE uint32_t LL_GPIO_AF_IsEnabledRemap_TIM4(void) {
		return (READ_BIT(AFIO->MAPR, AFIO_MAPR_TIM4_REMAP)
	...
	__STATIC_INLINE uint32_t LL_GPIO_AF_IsEnabledRemap_PD01(void) {
		return (READ_BIT(AFIO->MAPR, AFIO_MAPR_PD01_REMAP)
