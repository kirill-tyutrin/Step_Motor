	...
	if (tmp_hal_status == HAL_OK) {
		...
		if (HAL_IS_BIT_CLR(hadc->Instance->CR1, ADC_CR1_JAUTO)) {
			...
			ADC_NONMULTIMODE_OR_MULTIMODEMASTER(hadc)) {
				...
				SET_BIT(hadc->Instance->CR2,
						(ADC_CR2_JSWSTART | ADC_CR2_JEXTTRIG));
			} else {
...
HAL_StatusTypeDef HAL_ADCEx_InjectedConfigChannel(ADC_HandleTypeDef *hadc,
	...
	if (hadc->Init.ScanConvMode == ADC_SCAN_DISABLE) {
		...
		if (sConfigInjected->InjectedRank
			...
			MODIFY_REG(hadc->Instance->JSQR,
					...
					ADC_JSQR_JL_SHIFT(sConfigInjected->InjectedNbrOfConversion) | ADC_JSQR_RK_JL(sConfigInjected->InjectedChannel, sConfigInjected->InjectedRank, sConfigInjected->InjectedNbrOfConversion));
		} else {
			/* Clear the old SQx bits for the selected rank */
			MODIFY_REG(hadc->Instance->JSQR,
	...
	case 1:
		...
		MODIFY_REG(hadc->Instance->JOFR1, ADC_JOFR1_JOFFSET1,
				sConfigInjected->InjectedOffset);
		break;
	case 2:
		/* Set injected channel 2 offset */
		MODIFY_REG(hadc->Instance->JOFR2, ADC_JOFR2_JOFFSET2,
				sConfigInjected->InjectedOffset);
		break;
	case 3:
		/* Set injected channel 3 offset */
		MODIFY_REG(hadc->Instance->JOFR3, ADC_JOFR3_JOFFSET3,
				sConfigInjected->InjectedOffset);
		break;
	case 4:
	default:
		MODIFY_REG(hadc->Instance->JOFR4, ADC_JOFR4_JOFFSET4,
				sConfigInjected->InjectedOffset);
		break;
	}
